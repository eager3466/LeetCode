...
public class AddTwoNumbers {
     ...
     *
     *
     public data.ListNode addTwoNumbers(data.ListNode l1, data.ListNode l2) {
         data.ListNode dummyHead = new data.ListNode(0);
         data.ListNode p = l1, q = l2, curr = dummyHead;
         int carry = 0;
         while (p != null || q != null) {
            ...
            int y = (q != null) ? q.val : 0;
             int sum = carry + x + y;
             carry = sum / 10;
             curr.next = new data.ListNode(sum % 10);
             curr = curr.next;
            if (p != null) p = p.next;
            if (q != null) q = q.next;
         }
         if (carry > 0) {
            curr.next = new data.ListNode(carry);
         }
